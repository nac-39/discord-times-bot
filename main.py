...
TIMES_CATEGORY_NAME = os.getenv("TIMES_CATEGORY_NAME")
print(f"TIMES_CATEGORY_NAME: {TIMES_CATEGORY_NAME}")
...
class CreateTimesModal(Modal, title=f"新しい{TIMES_PREFIX}チャンネルの作成"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if TIMES_CATEGORY_NAME:
            target_category = discord.utils.get(guild.categories, name=TIMES_CATEGORY_NAME)
            if not target_category:
                await interaction.response.send_message(f"エラー: カテゴリ `{TIMES_CATEGORY_NAME}` が見つかりませんでした。", ephemeral=True)
                return
...
class TimesControlPanel(View):
    ...
    async def join_leave_button(self, interaction: discord.Interaction, button: Button):
        all_channels = sorted([
            ch for ch in interaction.guild.text_channels if ch.name.startswith(TIMES_PREFIX_UNDERBAR)
...
async def post_panel(interaction: discord.Interaction):
    embed = discord.Embed(
        ...
        description=(
            ...
            f"👇 **他のTimesに参加/退出する**: 既存の{TIMES_PREFIX}チャンネルに参加したり、退出したりします。"
        ),
...
async def times_channel_autocomplete(
    ...
    channels = [
        ...
        if channel.name.startswith(TIMES_PREFIX_UNDERBAR) and current.lower() in channel.name.lower()
    ]
    # 候補を25個に制限
    return [
